...
class lib_catalog
{
    ...
    public $tpl_node           = '<a href="{url}" class="{css_class}">{name}</a>';
	...
	function __construct($table_catalog = '', $table_catalog_cash = '', $table_catalog_link = '',  $tpl_node  = '', $menu_id=0)
	{
	...
	function getCash()
	{
        ...
        $sql = 'SELECT `cash` FROM `'.$this->table_catalog_cash.'` WHERE menu_id='.$this->menu_id;

        $result =  $this->db->query($sql);

        $html = $this->db->fetch_array($result);
	...
	function edit()
	{
		...
		else
		{
        	...
        	lib_javascript::add(self::PATH().'/catalog.js');
         }
	...
	function addcat($parent = 0, $inputs = null, $function = false)
	{
		...
		else
		{
            ...
            $list = array
         	...
         	$this->db->insert_array($list,$this->table_catalog);

            if(!empty($function)) $function($this->db->insert_id());
	...
	function editcat($cat = 0, $inputs = false, $function = false)
	{
		...
		else
		{
         	...
         	$this->db->update_array($list,$this->table_catalog,'id_catalog = '.intval($cat));
	...
	function view_admin($parent_id = 0, $prefix = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
	{
        ...
        $sql = "SELECT id_catalog, name, parent, pos, menu_id, no_active_item  FROM ".
            $this->table_catalog." WHERE menu_id = ".$this->menu_id." AND parent = '$parent_id' ORDER BY pos ";
	...
	function select_catalog($parent_id = 0, $prefix = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
	{
        ...
        $sql = "SELECT id_catalog, name, parent  FROM ".$this->table_catalog." WHERE menu_id = ".$this->menu_id." AND  parent = '$parent_id' ORDER BY pos";
	...
	function get_catalog($parent_id = 0)
	{
        ...
        $sql = "SELECT id_catalog, name, parent, no_active_item, url  FROM ".$this->table_catalog." WHERE menu_id='".$this->menu_id."' AND parent = '$parent_id' ORDER BY pos";
        ...
        while ($row = $this->db->fetch_array($result))
        {
        	...
        	$size_node =  $this->db->num_rows($result2);

            $css = '';
            $url = '/'.$row['url'].'.html';

            if($row['no_active_item'] == 1) {
                $css = "no-active";
                $url = '#';
            }

            $tpl ='';
            $tpl = str_replace('{url}',$url, $this->tpl_node);

            $tpl = str_replace('{name}',$row['name'],$tpl);
            $tpl = str_replace('{css_class}',$css,$tpl);



            if(!isset($_catalog_out[$this->catalog_id])) $_catalog_out[$this->catalog_id] = "";

        	$_catalog_out[$this->catalog_id] .= '<li>'.$tpl."\n";

            if($size_node > 0)  $_catalog_out[$this->catalog_id] .= "<ul>\n";
			if($size_node > 0)  $this->get_catalog( $row['id_catalog']);

			if($size_node > 0)  $_catalog_out[$this->catalog_id] .= "</ul>\n";
            $_catalog_out[$this->catalog_id] .= '</li>';
        }
	...
	function setCash()
	{
        ...
        $sql = 'SELECT id_cash FROM '.$this->table_catalog_cash. ' WHERE menu_id = ' . $this->menu_id;

		$result    =  $this->db->query($sql);

        if($this->db->num_rows($result) > 0)
        {
            $this->db->update_array
         	(
         		array
         		(
         			'cash' => $html
