...
class lib_mUploadImages
{
    ...
    private function upload()
    {
        ...
        if($upload->error == 0 )
        {
            ...
            $img = new lib_image($this->conf['path']."/".$upload->file);
...
//            return json_encode($json);
            ...
            return "<script>location.href = parent.location.href + '&medit=widget_form&widget=images&widget_id='+$widget_id+'&widget_action=edit';</script>";
    ...
    {
        $this->head();
        $url = $this->conf['upload_script'];
        if (stripos($url, "muploadimages")===false) {
            $url .= "&class=muploadimages";
        }
        if (stripos($url, "uli_upload")===false) {
            $url .= "&uli_upload";
        }

        $conf = $this->conf;
        $top = '<form action="'.$url.'" method="post" enctype="multipart/form-data">';
        $top .= $this->upload_button();
    ...
    private function upload_button()
    {

        return  '
                <input type="file" type="file" name="Filedata" />
...
//        return self::html_wrap($span, "", array('id' => "Buttons"));
    ...
    {
        $width  = self::input('number', array('name' => 'max_width', 'class' => $id.'_width', 'step'  => 10, 'value'=>$width) );
        $height = self::input('number', array('name' => 'max_height', 'class' => $id.'_height', 'step' => 10, 'value'=>$height) );
    ...
    private function getImagesDB()
    {
        ...
        if(isset($conf['Limit']) && !is_null($conf['Limit']) )
        {
            $sql .= ' LIMIT ';
    ...
    {
        lib_css::add($this->conf['js_path']."/css/style1.css");
        lib_javascript::add($this->conf['js_path']."/js/jquery-1.3.2.min.js");
        lib_javascript::add($this->conf['js_path']."/js/jquery-ui.min.js");
//       lib_javascript::add($this->conf['js_path']."/js/swfupload.js");
//        lib_javascript::add($this->conf['js_path']."/js/swfupload.cookies.js");
        lib_javascript::add($this->conf['js_path']."/js/jquery-migrate-1.2.1.js");


        lib_javascript::add($this->conf['js_path']."/js/script.js");

        lib_javascript::add("MFWPath = '".$this->conf['js_path']."'");
        lib_javascript::add("$(function(){ BindSWFUpload('".$this->conf['upload_script']."',
