 ...
 class lib_datebase
 {
	...
	function query()
	{
        ...
        if($this->debugging)
        {
    		...
    		$count_row=$this->num_rows($result);

            //lib_javascript::add("console.log('".$query." ".$count_row." row in set (".$times.")');");
    ...
    /*
	 *  num_rows($result) - Возвращает количество рядов результата запроса
	...
	/*
	 *  fetch_array ( resource $result [, int $result_type ] ) - Возвращает массив с обработанным рядом результата запроса,
	 *  или FALSE, если рядов больше нет.
     *  Второй опциональный аргумент result_type в функции fetch_array() - константа и может
	...
	/*
	 *  fetch_object ( resource $result) - Обрабатывает ряд результата запроса и возвращает объект
	 */
    ...
    /*
	 *  insert_id() - Возвращает численный код ошибки выполнения последней операции
	 */
    ...
    function insert_array($list,$table)
    {
           $cols="";
           $values="";
    ...
    function update_array($list,$table, $where="")
    {
    ...
    function multy_query($sql)
    {
     ...
     public function get_row($sql)
     {
         ...
         if(!$this->num_rows($result)) return false;
	 ...
	 public function list_fields($table)
	 {
		 ...
		 $result = $this->query($sql);
		 $fields = array();
		 while($field = $this->fetch_array($result))
		 {
			 $fields[] = $field['Field'];
		 }
