...
class lib_form
{
    ...
    */
    ...
    private $wrap_types = array('fieldset', 'inline', 'tab', 'multiline');
    ...
    function __construct($conf = "")
    {
        ...
        $this->conf['errors'] = array_merge(valid::$errors, $this->conf['errors']);
    ...
    public function add_items($items, $default_conf = null)
    {
        foreach($items as $it)
        {
    ...
    function data($name)
    {
        if(is_null($this->data))
        {
            foreach($this->items as $it)
            {
                ...
                if(!isset($it->item['name']) || is_null($it->item['name'])) continue;
    ...
    private function format($it)
    {
        ...
        switch($format['type'])
        {
            ...
            default:
                ...
                if(!is_array($value) && (!isset($format['html']) || !$format['html']) )
                    $value = strip_tags($value);
                if(!is_array($value) && isset($format['trim']) && $format['trim'])       $value = trim($value);
                if(!is_array($value) && isset($format['upper']) && $format['upper'])     $value = strtoupper($value);
                if(!is_array($value) && isset($format['lower']) && $format['lower'])     $value = strtolower($value);
                if(!is_array($value) && isset($format['for_sql']) && $format['for_sql']) $value = mysql_real_escape_string($value);
    ...
    public function generate($valid = false, $full = false)
    {
        ...
        foreach($this->items as $it)
        {
            ...
            if($full)
            {
                ...
                {
                    $value = (isset($_POST[$it->item['name']])) ? $_POST[$it->item['name']] : "";
    ...
    {
        /* проверить! */
        //if(!empty($it->javascript)) lib_javascript::add($it->javascript);
