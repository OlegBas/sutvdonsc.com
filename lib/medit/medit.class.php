...
class lib_medit
{
        ...
        switch($request['medit'])
        {
            ...
            case "pos_widget":
                ...
                (
                    ...
                    $out = $this->exchange_pos_widget($request['id1'], $request['pos1'],
    ...
    private function js($file)
    {
        ...
        lib_javascript::add($this->conf['path_js'].'/'.$file);
    ...
    private function theme($theme)
    {
        ...
        lib_css::add($this->conf['theme_dir'].'/'.$theme.'/style.css');
    }
    ...
    /**
     * функция получает параметры выиджита из бд
     * @param $id
     * @return mixed
     */
    private function getParameters($id)
    {
    ...
    private function widget_form($widget, $id = null)
    {
        ...
        lib_css::add($conf['theme_dir'].'/'.$conf['theme']."/form.css");
        $html  = '';
        $form_items = array();
...
*/
    ...
    private function get_pos($id)
    {
        ...
        $pos       = $this->db->fetch_array($result);
    ...
    public  function  max_pos()
    {
        ...
        $posmax       = $this->db->fetch_array($result);
    ...
    public function save_element_art($widget, $parameters, $id = null)
    {
        ...
        if(!$id)
        {
            ...
            $pos        = $this->max_pos() + 1;
                ...
                $this->db->insert_array($list, $this->conf['db_table_elements_art']);
        ...
        (
            ...
            $this->db->update_array($list, $this->conf['db_table_elements_art'], ' id =' . $id);
