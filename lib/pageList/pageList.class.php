<?
  class lib_pageList
 {
    ...
    private $db;
 	...
 	function __construct($slqSelect, $numRow = 10)
 	{
        ...
        $this->db =  $DB;
 		...
 		if( !isset($_GET[$this->getVariableName]) ||
			!preg_match("/^\d+$/",$_GET[$this->getVariableName]) )
 		{
        ...
        $this->numRows = $this->db->fetch_array( $this->db->query($sql));
	...
	{
		$thisUrl = lib_path::this_url();
		if(stripos($thisUrl, $this->getVariableName) !== false) {

			$url = lib_path::replace(
				array($this->getVariableName => '{PAGE}'),
				$thisUrl
			);
		} else {
			$url = lib_path::add_param($this->getVariableName . "={PAGE}");
		}
