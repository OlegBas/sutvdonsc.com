  ...
  */
  $('#calendar').on('click', function(event) {
    ...
    if ($(target).attr('class') === 'active day' || $(target).attr('class') === 'active disabled day')
    {
      ...
      var url = "/index.php?action=events_ajax&module=articles&type=ajax&date="+$(target).attr('data-date');


      $('#calendar_popup_area').load(url, function (){$('#calendar_popup_area').show(300);});
