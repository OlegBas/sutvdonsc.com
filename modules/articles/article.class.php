...
class article extends lib_crud
{
    ...
    public function event_add($id, $form)
    {
        ...
        lib_header::location("/article.html?id=".$id, 'top');
...
*/
    ...
    public function event_edit($id, $form)
    {
        ...
        /* массив с категориями в которых есть запись о позиции */
        $sql = 'SELECT * FROM ' . DB_ARTICLE_POSITION . ' WHERE `article_id` = ?';
        $result = $this->db->query($sql, $id);

        $catalogs = array();

        while($row = $this->db->fetch_array($result)) $catalogs[] = $row['catalog_id'];


        $pos = new lib_position();

        foreach ($_POST['catid'] as $cat)
        {
            if(!in_array($cat, $catalogs )) $pos->newItem($id, $cat);
        }
